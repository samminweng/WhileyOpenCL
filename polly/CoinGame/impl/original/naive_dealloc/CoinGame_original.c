...
int main(int argc, char** args){
	...
	//assign %3 = %11  : int[]
	_DEALLOC(moves);
	_COPY_1DARRAY_int64_t(moves, _11);
	_ADD_DEALLOC(moves);
	//invoke (%12) = (%3, %2) CoinGame_original:findMoves : function(int[],int)->(int[])
	{
		void* moves_tmp;
		_DEALLOC(_12);
		_12 = findMoves(_COPY_1DARRAY_PARAM_int64_t(moves), false, n, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_12));
		_CALLER_DEALLOC(_12, moves, "true-true-false" , "findMoves");
		_ADD_DEALLOC(_12);
	}
	//assign %3 = %12  : int[]
	_DEALLOC(moves);
	_COPY_1DARRAY_int64_t(moves, _12);
	_ADD_DEALLOC(moves);
	//const %13 = 1 : int
	_13 = 1;
	//sub %14 = %2, %13 : int
