#include "Array_populatearray.h"
int64_t* append(_DECL_1DARRAY_PARAM(items), _DECL_DEALLOC_PARAM(items), int64_t item, _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
...
int64_t* appendArray(_DECL_1DARRAY_PARAM(data), _DECL_DEALLOC_PARAM(data), _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		//add %11 = %9, %10 : int
		_11=_9+_10;
		//invoke (%8) = (%1, %11) Array_populatearray:append : function(int[],int)->(int[])
		{
			_DEALLOC(_8);
			_8 = append(_1DARRAY_PARAM(output), false, _11, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_8));
...
int64_t* fastAppendArray(_DECL_1DARRAY_PARAM(data), _DECL_DEALLOC_PARAM(data), _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	_DECL_1DARRAY(output);
	_DECL_DEALLOC(output);
	int64_t pos = 0;
	int64_t size = 0;
	_DECL_1DARRAY(output_1);
	_DECL_DEALLOC(output_1);
	int64_t size_1 = 0;
	int64_t _6 = 0;
	int64_t _7 = 0;
	int64_t _8 = 0;
...
blklab4:;
	...
	_TRANSFER_DEALLOC(output_1, _22);
	//const %23 = 0 : int
	_23 = 0;
	//assign %5 = %23  : int
	size_1 = _23;
...
blklab6:;
	...
	_TRANSFER_DEALLOC(output, output_1);
	//return %1
	_DEALLOC(data);
	_DEALLOC(output_1);
	_DEALLOC(_11);
...
int main(int argc, char** args){
	...
	//assign %2 = %1  : int
	max = *n;
	//invoke (%9) = (%2) Array_populatearray:init : function(int)->(int[])
	{
		_DEALLOC(_9);
	...
	//invoke (%10) = (%3) Array_populatearray:fastAppendArray : function(int[])->(int[])
	{
		_DEALLOC(_10);
		_10 = fastAppendArray(_1DARRAY_PARAM(data), false, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_10));
		_RETAIN_DEALLOC(data, "false-false-false" , "fastAppendArray");
		_ADD_DEALLOC(_10);
	}
	//assign %4 = %10  : int[]
	_DEALLOC(arr1);
	_UPDATE_1DARRAY(arr1, _10);
	_TRANSFER_DEALLOC(arr1, _10);
