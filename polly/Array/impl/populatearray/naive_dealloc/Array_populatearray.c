...
int64_t* appendArray(_DECL_1DARRAY_PARAM(data), _DECL_DEALLOC_PARAM(data), _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		//invoke (%8) = (%1, %11) Array_populatearray:append : function(int[],int)->(int[])
		{
			void* output_tmp;
			_DEALLOC(_8);
			_8 = append(_COPY_1DARRAY_PARAM_int64_t(output), true, _11, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_8));
...
blklab4:;
	...
	//assign %4 = %22  : int[]
	_DEALLOC(output_1);
	_COPY_1DARRAY_int64_t(output_1, _22);
	_ADD_DEALLOC(output_1);
	//const %23 = 0 : int
	_23 = 0;
	//assign %5 = %23  : int
	size_1 = _23;
...
int main(int argc, char** args){
	...
	//invoke (%10) = (%3) Array_populatearray:fastAppendArray : function(int[])->(int[])
	{
		void* data_tmp;
		_DEALLOC(_10);
		_10 = fastAppendArray(_COPY_1DARRAY_PARAM_int64_t(data), true, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_10));
		_CALLEE_DEALLOC(data, "false-false-false" , "fastAppendArray");
		_ADD_DEALLOC(_10);
	}
	//assign %4 = %10  : int[]
	_DEALLOC(arr1);
	_COPY_1DARRAY_int64_t(arr1, _10);
	_ADD_DEALLOC(arr1);
