...
blklab23:;
	...
	_ADD_OWNERSHIP(rows);
	_16 = matrix(width, height, _2DARRAY_PARAM(rows), false);
	_ADD_OWNERSHIP(_16);
	//return %16
	_FREE_STRUCT(r, Matrix);
	_FREE2DArray(rows);
...
int main(int argc, char** args){
	...
	_ADD_OWNERSHIP(A);
	_ADD_OWNERSHIP(B);
	_11 = multiply(_STRUCT_PARAM(A), false, _STRUCT_PARAM(B), false);
	_ADD_OWNERSHIP(_11);
