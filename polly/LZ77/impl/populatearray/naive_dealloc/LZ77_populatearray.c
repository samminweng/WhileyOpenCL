...
Match* findLongestMatch(BYTE* data, size_t data_size, _DECL_DEALLOC_PARAM(data), int64_t pos){
	...
	while(true){
		...
		//invoke (%14) = (%0, %6, %1) LZ77_populatearray:match : function(byte[],LZ77_populatearray:nat,LZ77_populatearray:nat)->(int)
		{
			void* data_tmp;
...
BYTE* compress(BYTE* data, size_t data_size, _DECL_DEALLOC_PARAM(data), _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		//invoke (%15) = (%0, %2) LZ77_populatearray:findLongestMatch : function(byte[],LZ77_populatearray:nat)->(LZ77_populatearray:Match)
		{
			void* data_tmp;
			_DEALLOC_STRUCT(_15, Match);
			_15 = findLongestMatch(_COPY_1DARRAY_PARAM_BYTE(data), true, pos);
...
//.blklab10
blklab10:;
	//invoke (%30) = (%1, %3) LZ77_populatearray:populate : function(byte[],LZ77_populatearray:nat)->(byte[])
	{
		void* output_tmp;
		_DEALLOC(_30);
		_30 = populate(_COPY_1DARRAY_PARAM_BYTE(output), true, size, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_30));
		_CALLEE_DEALLOC(output, "false-false-false" , "populate");
		_ADD_DEALLOC(_30);
	}
	//assign %1 = %30  : byte[]
	_DEALLOC(output);
...
BYTE* decompress(BYTE* data, size_t data_size, _DECL_DEALLOC_PARAM(data), _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		//invoke (%23) = (%1, %4) LZ77_populatearray:append_byte : function(byte[],byte)->(byte[])
		{
			void* output_tmp;
			_DEALLOC(_23);
...
blklab16:;
		...
		while(true){
			...
			//assign %4 = %29  : byte
			item = _29;
			//invoke (%30) = (%1, %4) LZ77_populatearray:append_byte : function(byte[],byte)->(byte[])
			{
				void* output_tmp;
				_DEALLOC(_30);
...
int main(int argc, char** args){
	...
	//invoke (%20) = (%2) LZ77_populatearray:compress : function(byte[])->(byte[])
	{
		void* data_tmp;
		_DEALLOC(_20);
		_20 = compress(_COPY_1DARRAY_PARAM_BYTE(data), true, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_20));
		_CALLEE_DEALLOC(data, "false-false-false" , "compress");
		_ADD_DEALLOC(_20);
	}
	//assign %3 = %20  : byte[]
	_DEALLOC(compress_data);
	_COPY_1DARRAY_BYTE(compress_data, _20);
	...
	//invoke (%30) = (%3) LZ77_populatearray:decompress : function(byte[])->(byte[])
	{
		void* compress_data_tmp;
		_DEALLOC(_30);
		_30 = decompress(_COPY_1DARRAY_PARAM_BYTE(compress_data), true, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_30));
		_CALLEE_DEALLOC(compress_data, "false-false-false" , "decompress");
		_ADD_DEALLOC(_30);
	}
	//assign %4 = %30  : byte[]
	_DEALLOC(decompress_data);
	_COPY_1DARRAY_BYTE(decompress_data, _30);
	...
	//assert
	{
		...
		//ifeq %43, %44 goto blklab20 : int
		if(_43==_44){goto blklab20;}
		//fail
		fprintf(stderr,"fail");
		exit(-1);
//.blklab20
