...
BYTE* compress(BYTE* data, size_t data_size, _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		{
			void* data_tmp;
			_17 = findLongestMatch(_COPY_1DARRAY_PARAM_BYTE(data), pos);
		}
...
blklab12:;
	//assign %1 = %7  : byte[]
	_COPY_1DARRAY_BYTE(output, output2);
	//return %1
	_UPDATE_1DARRAYSZIE_PARAM_CALLBYREFERENCE(output);
	return output;
	//return
...
int main(int argc, char** args){
	...
	{
		void* data_tmp;
		_19 = compress(_COPY_1DARRAY_PARAM_BYTE(data), _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_19));
	}
	//assign %3 = %19  : byte[]
