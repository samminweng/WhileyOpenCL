...
BYTE* compress(BYTE* data, size_t data_size, _DECL_DEALLOC_PARAM(data), _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		//invoke (%15) = (%0, %2) LZ77_populatearray:findLongestMatch : function(byte[],LZ77_populatearray:nat)->(LZ77_populatearray:Match)
		{
			...
			_15 = findLongestMatch(_1DARRAY_PARAM(data), false, pos);
...
blklab10:;
	...
	{
		_DEALLOC(_30);
		_30 = populate(_1DARRAY_PARAM(output), false, size, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_30));
		_RETAIN_DEALLOC(output, "false-false-false" , "populate");
		_ADD_DEALLOC(_30);
	}
	//assign %1 = %30  : byte[]
	_DEALLOC(output);
