...
BYTE* write_u1(BYTE* bytes, size_t bytes_size, _DECL_DEALLOC_PARAM(bytes), int64_t u1, _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	//invoke (%3) = (%0, %4) LZ77_populatearray:append_byte : function(byte[],byte)->(byte[])
	{
		_DEALLOC(_3);
...
BYTE* compress(BYTE* data, size_t data_size, _DECL_DEALLOC_PARAM(data), _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		//invoke (%17) = (%0, %2) LZ77_populatearray:findLongestMatch : function(byte[],LZ77_populatearray:nat)->(LZ77_populatearray:Match)
		{
			_DEALLOC_STRUCT(_17, Match);
			_17 = findLongestMatch(_1DARRAY_PARAM(data), false, pos);
...
blklab12:;
	//assign %1 = %7  : byte[]
	_DEALLOC(output);
	_UPDATE_1DARRAY(output, output2);
	_TRANSFER_DEALLOC(output, output2);
	//return %1
	_DEALLOC(data);
	_DEALLOC_STRUCT(m, Match);
	_DEALLOC(output2);
...
int main(int argc, char** args){
	...
	//invoke (%19) = (%2) LZ77_populatearray:compress : function(byte[])->(byte[])
	{
		_DEALLOC(_19);
		_19 = compress(_1DARRAY_PARAM(data), false, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_19));
	...
	_TRANSFER_DEALLOC(compress_data, _19);
