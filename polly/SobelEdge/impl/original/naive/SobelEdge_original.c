	...
	while(true){
		...
		while(true){
			...
			//invoke (%47) = (%0, %1, %2, %8, %9, %6) SobelEdge_original:convolution : function(int[],int,int,int,int,int[])->(int)
			{
				void* pixels_tmp;
				void* v_sobel_tmp;
				_47 = convolution(_COPY_1DARRAY_PARAM_LONGLONG(pixels), width, height, x, y, _COPY_1DARRAY_PARAM_LONGLONG(v_sobel));
			}
			//assign %11 = %47  : int
			v_g = _47;
			//invoke (%48) = (%0, %1, %2, %8, %9, %7) SobelEdge_original:convolution : function(int[],int,int,int,int,int[])->(int)
			{
				void* pixels_tmp;
				void* h_sobel_tmp;
				_48 = convolution(_COPY_1DARRAY_PARAM_LONGLONG(pixels), width, height, x, y, _COPY_1DARRAY_PARAM_LONGLONG(h_sobel));
			}
			//assign %12 = %48  : int
			h_g = _48;
...
int main(int argc, char** args){
	...
	//invoke (%20) = (%6, %3, %4) SobelEdge_original:sobelEdgeDetection : function(int[],int,int)->(int[])
	{
		...
		_20 = sobelEdgeDetection(_COPY_1DARRAY_PARAM_LONGLONG(pixels), width, height, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_20));
	}
	//assign %7 = %20  : int[]
	_COPY_1DARRAY_LONGLONG(newPixels, _20);
	//fieldload %21 = %0 out : {int[][] args,{method(any)->() print,method(int[])->() print_s,method(any)->() println,method(int[])->() println_s} out}
	...
	{
		void* pixels_tmp;
		printImage(stdout, _COPY_1DARRAY_PARAM_LONGLONG(pixels), width, height);
	}
	...
	{
		void* newPixels_tmp;
		printImage(stdout, _COPY_1DARRAY_PARAM_LONGLONG(newPixels), width, height);
	}
//.blklab16
blklab16:;
	//return
