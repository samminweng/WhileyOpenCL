...
/**
 * Generate an 2D array and initialize each subarray with given array
 */
long long** gen2DArray(long long* arr, long long arr_size, long long size){
	long long** _2DArray = NULL;
	long long i = 0;
	// Allocate the array
	_2DArray = (long long**)malloc(size*sizeof(long long*));
	if(_2DArray == NULL){
		printf("fail to allocate the memory at genArray function in Util.c\n");
		exit(-2);
	}
	//memcpy(_2DArray, arr, arr_size * size*sizeof(long long));
	for(i=0;i<size;i++){
		// Copy the input array and assign it to matrix.
		_2DArray[i] = clone(arr, arr_size);
	}
	return _2DArray;
}
...
long long** clone2DArray(long long **arr, long long arr_size){
	long long **newMatrix = NULL;
	newMatrix = (long long**) malloc(arr_size*sizeof(long long));
	if(newMatrix == NULL){
		printf("fail to malloc at clone2DArray function in Util.c\n");
		exit(-2);
	}
	memcpy(newMatrix, arr, arr_size*sizeof(long long));
	return newMatrix;
}
