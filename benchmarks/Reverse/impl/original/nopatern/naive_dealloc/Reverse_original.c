...
int64_t* reverse(_DECL_1DARRAY_PARAM(arr), _DECL_DEALLOC_PARAM(arr), _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	_DECL_1DARRAY(_1);
	_DECL_DEALLOC(_1);
	int64_t i = 0;
	_DECL_1DARRAY(r);
...
blklab5:;
	//invoke (%15) = (%4) Reverse_original:reverse : function(int[])->(int[])
	{
		void* arr_tmp;
		_DEALLOC(_15);
		_15 = reverse(_COPY_1DARRAY_PARAM_int64_t(arr), true, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_15));
