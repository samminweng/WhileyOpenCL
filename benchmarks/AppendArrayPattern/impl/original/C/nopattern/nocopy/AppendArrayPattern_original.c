...
BYTE* _comp_(BYTE* data, size_t data_size, _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		{
			_14 = _append_(_1DARRAY_PARAM(output), length, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_14));
		}
		//assign %1 = %14  : byte[]
		_UPDATE_1DARRAY(output, _14);
		//invoke (%15) = (%1, %3) AppendArrayPattern_original:append : function(byte[],byte)->(byte[])
		{
			_15 = _append_(_1DARRAY_PARAM(output), item, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_15));
		}
		//assign %1 = %15  : byte[]
...
int main(int argc, char** args){
	...
	{
		_11 = _comp_(_1DARRAY_PARAM(data), _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_11));
	}
	//assign %4 = %11  : byte[]
