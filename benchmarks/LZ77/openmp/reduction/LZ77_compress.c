...
struct Compare { int64_t len; int64_t offset;};
...
Match* _findLongestMatch_(BYTE* data, size_t data_size, _DECL_DEALLOC_PARAM(data), int64_t pos){
	...
	#pragma omp
	{
		struct Compare local_max;
		local_max.len = 0;
		local_max.offset = 0;
		// Convert while-loop into for-loop
		#pragma omp parallel for reduction(maximum:local_max)
		for(offset = start;offset<pos;offset++){
			...
			if(len > local_max.len){
				local_max.len = len;
				local_max.offset = offset;
			}
		}
		...
		//#pragma omp critical
		{
			...
			//ifle %7, %4 goto blklab5 : int
			if(local_max.len<=bestLen){goto blklab5;}
			//sub %15 = %1, %6 : int
			_15=pos-local_max.offset;
			//assign %3 = %15  : int
			// isCopyEliminated = true
			bestOffset = _15;
			//assign %4 = %7  : int
			// isCopyEliminated = false
			bestLen = local_max.len;
			blklab5:;
			//.blklab5
		}
	}
