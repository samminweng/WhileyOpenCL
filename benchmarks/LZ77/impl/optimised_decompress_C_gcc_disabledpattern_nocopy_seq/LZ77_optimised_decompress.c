...
BYTE* _decompress_(BYTE* data, size_t data_size, _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		//invoke (%28) = (%2, %3, %7) LZ77_optimised_decompress:opt_append : function(byte[],LZ77_optimised_decompress:nat,byte)->(LZ77_optimised_decompress:MyArrayList)
		{
			...
			_28 = _opt_append_(_1DARRAY_PARAM(items), items_length, item);
		}
...
blklab11:;
		...
		while(true){
			...
			{
				// isCopyEliminated of '_2' = true
				_36 = _opt_append_(_1DARRAY_PARAM(items), items_length, item);
			}
...
blklab9:;
	...
	{
		// isCopyEliminated of '_2' = true
		_41 = _resize_(_1DARRAY_PARAM(items), items_length, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_41));
	}
	//assign %1 = %41  : byte[]
	// isCopyEliminated = true
