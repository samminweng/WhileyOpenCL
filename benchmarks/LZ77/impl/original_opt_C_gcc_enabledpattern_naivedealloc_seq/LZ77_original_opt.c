...
Match* _findLongestMatch_(BYTE* data, size_t data_size, _DECL_DEALLOC_PARAM(data), int64_t pos){
	...
	while(true){
		...
		{
			void* data_tmp;
			_14 = _match_(_COPY_1DARRAY_PARAM_BYTE(data), true, offset, pos);
...
blklab15:;
	//assert
	{
		//ifle %28, %26 goto blklab29 : int
		if(_28<=_26){goto blklab29;}
		//fail
		fprintf(stderr,"fail");
		exit(-1);
//.blklab29
blklab29:;
	...
	//invoke (%33) = (%1, %28) LZ77_original_opt:resize : function(byte[],int)->(byte[])
	{
		void* output_tmp;
		_DEALLOC(_33);
		_33 = _resize_(_COPY_1DARRAY_PARAM_BYTE(output), true, _28, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_33));
...
BYTE* _decompress_(BYTE* data, size_t data_size, _DECL_DEALLOC_PARAM(data), _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		//invoke (%26) = (%2, %3, %6) LZ77_original_opt:opt_append : function(byte[],LZ77_original_opt:nat,byte)->(byte[])
		{
			void* items_tmp;
			_DEALLOC(_26);
			_26 = _opt_append_(_COPY_1DARRAY_PARAM_BYTE(items), false, items_length, item, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_26));
			_CALLER_DEALLOC(_26, items, "true-true-false" , "opt_append");
		}
		//assign %2 = %26  : byte[]
		_DEALLOC(items);
...
blklab25:;
		...
		while(true){
			...
			//invoke (%34) = (%2, %3, %6) LZ77_original_opt:opt_append : function(byte[],LZ77_original_opt:nat,byte)->(byte[])
			{
				void* items_tmp;
				_DEALLOC(_34);
				_34 = _opt_append_(_COPY_1DARRAY_PARAM_BYTE(items), false, items_length, item, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_34));
				_CALLER_DEALLOC(_34, items, "true-true-false" , "opt_append");
			}
			//assign %2 = %34  : byte[]
			_DEALLOC(items);
...
blklab23:;
	//invoke (%39) = (%2, %3) LZ77_original_opt:resize : function(byte[],int)->(byte[])
	{
		void* items_tmp;
		_DEALLOC(_39);
		_39 = _resize_(_COPY_1DARRAY_PARAM_BYTE(items), true, items_length, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_39));
...
int main(int argc, char** args){
	...
	//invoke (%23) = (%2) LZ77_original_opt:compress : function(byte[])->(byte[])
	{
		void* data_tmp;
		_DEALLOC(_23);
		_23 = _compress_(_COPY_1DARRAY_PARAM_BYTE(data), true, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_23));
