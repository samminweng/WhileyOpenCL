...
function compress(byte[] data) -> (byte[] output):
    nat pos = 0
    // Pre-allocate the output array of bytes
    output = [0b;2*|data|]// Pre−allocates 2x input array size
    int size = 0 // Actual array size
    ...
    while pos < |data|:
        ...
        output[size]= length
        size = size + 1
    // Reduce output array to actual size
