...
blklab20:;
	//invoke (%22) = (%5, %2, %3) SobelEdge_large:sobelEdgeDetection : function(byte[],int,int)->(byte[])
	{
		void* tmp_pixels;
		_COPY_1DARRAY_PARAM(pixels, tmp_pixels, BYTE);
		_22 = _sobelEdgeDetection_(tmp_pixels, pixels_size, width, height, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_22));
	}
	//assign %7 = %22  : byte[]
	_COPY_1DARRAY_BYTE(newPixels, _22);
	//fieldload %23 = %0 out : {int[][] args,{method(any)->() print,method(int[])->() print_s,method(any)->() println,method(int[])->() println_s} out}
