...
int main(int argc, char** args){
	...
	_NEW1DARRAY_DEALLOC(_15, _14, size, BYTE);
	//assign %5 = %15  : byte[]
	_ADD_DEALLOC(pixels, _15, BYTE);
	//const %16 = 0 : int
	_16 = 0;
	//assign %6 = %16  : int
	i = _16;
...
blklab20:;
	...
	{
		_FUNCTIONCALL_COPY_PRE(_29, pixels, 0, "liveness: pixels = false, readonly: pixels = true, return:pixels = NEVER_RETURN" , "sobelEdgeDetection");
		_DEALLOC(_29);
		void* tmp_pixels;
		_COPY_1DARRAY_PARAM(pixels, tmp_pixels, BYTE);
		_29 = _sobelEdgeDetection_(tmp_pixels, pixels_size, width, height, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_29));
		_29_dealloc = true;
		free(tmp_pixels);
	}
	//assign %7 = %29  : byte[]
	_ADD_DEALLOC(newPixels, _29, BYTE);
