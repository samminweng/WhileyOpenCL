...
int64_t _convolution_(BYTE* pixels, size_t pixels_size, _DECL_DEALLOC_PARAM(pixels), int64_t width, int64_t height, int64_t xCenter, int64_t yCenter, _DECL_1DARRAY_PARAM(kernel), _DECL_DEALLOC_PARAM(kernel)){
	...
	while(true){
		...
		//sub %22 = %21, %9 : int
		_22=_21-kernelHalf;
		//invoke (%20) = (%22, %2) SobelEdge_nomod:wrap : function(int,int)->(int)
		{
		...
		while(true){
			...
			//sub %26 = %25, %9 : int
			_26=_25-kernelHalf;
			//invoke (%24) = (%26, %1) SobelEdge_nomod:wrap : function(int,int)->(int)
			{
...
BYTE* _sobelEdgeDetection_(BYTE* pixels, size_t pixels_size, _DECL_DEALLOC_PARAM(pixels), int64_t width, int64_t height, _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		while(true){
			...
			// isCopyEliminated = true
			pos = _46;
			//invoke (%47) = (%0, %1, %2, %8, %9, %6) SobelEdge_nomod:convolution : function(byte[],int,int,int,int,int[])->(int)
			{
			...
			// isCopyEliminated = true
			v_g = _47;
			//invoke (%48) = (%0, %1, %2, %8, %9, %7) SobelEdge_nomod:convolution : function(byte[],int,int,int,int,int[])->(int)
			{
				// isCopyEliminated of '_0' = true
...
//.blklab20
blklab20:;
	//invoke (%22) = (%5, %2, %3) SobelEdge_nomod:sobelEdgeDetection : function(byte[],int,int)->(byte[])
	{
		_DEALLOC(_22);
