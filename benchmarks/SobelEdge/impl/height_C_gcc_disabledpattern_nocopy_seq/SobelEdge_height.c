...
BYTE* _sobelEdgeDetection_(BYTE* pixels, size_t pixels_size, int64_t width, int64_t height, _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		while(true){
			...
			// isCopyEliminated = true
			pos = _46;
			//invoke (%47) = (%0, %1, %2, %8, %9, %6) SobelEdge_height:convolution : function(byte[],int,int,int,int,int[])->(int)
			{
			...
			// isCopyEliminated = true
			v_g = _47;
			//invoke (%48) = (%0, %1, %2, %8, %9, %7) SobelEdge_height:convolution : function(byte[],int,int,int,int,int[])->(int)
			{
				// isCopyEliminated of '_0' = true
...
//.blklab20
blklab20:;
	//invoke (%22) = (%5, %2, %3) SobelEdge_height:sobelEdgeDetection : function(byte[],int,int)->(byte[])
	{
