	...
	while(true){
		...
		while(true){
			...
			//invoke (%47) = (%0, %1, %2, %8, %9, %6) SobelEdge_original:convolution : function(byte[],int,int,int,int,int[])->(int)
			{
				void* pixels_tmp;
				void* v_sobel_tmp;
				_47 = _convolution_(_COPY_1DARRAY_PARAM_BYTE(pixels), width, height, x, y, _COPY_1DARRAY_PARAM_int64_t(v_sobel));
			}
			//assign %11 = %47  : int
			v_g = _47;
			//invoke (%48) = (%0, %1, %2, %8, %9, %7) SobelEdge_original:convolution : function(byte[],int,int,int,int,int[])->(int)
			{
				void* pixels_tmp;
				void* h_sobel_tmp;
				_48 = _convolution_(_COPY_1DARRAY_PARAM_BYTE(pixels), width, height, x, y, _COPY_1DARRAY_PARAM_int64_t(h_sobel));
			}
			//assign %12 = %48  : int
			h_g = _48;
...
int main(int argc, char** args){
	...
	//ifis %2, null goto blklab19 : null|int
	...
	{
		void* pixels_tmp;
		_18 = _sobelEdgeDetection_(_COPY_1DARRAY_PARAM_BYTE(pixels), width, height, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_18));
	}
	//assign %7 = %18  : byte[]
	...
	{
		void* newPixels_tmp;
		_print_pbm_(stdout, width, height, _COPY_1DARRAY_PARAM_BYTE(newPixels));
	}
//.blklab19
blklab19:;
	//return
