...
int64_t* sliceAndSort(int64_t* items, size_t item_size, int start, int end);
...
int64_t* mergesort(int64_t* items, size_t item_size, int start, int end){
	...
	if(start+1 < end){
		...
		if(item_size>=1000){
			// LHS array
			lhs = cilk_spawn sliceAndSort(items, item_size, start, pivot);
		}else{
			lhs = sliceAndSort(items, item_size, start, pivot);
		}

		// RHS array
		if(item_size>=1000){
			rhs = cilk_spawn sliceAndSort(items, item_size, pivot, end);
		}else{
			rhs = sliceAndSort(items, item_size, pivot, end);
		}
...
int64_t* sliceAndSort(int64_t* items, size_t item_size, int start, int end){
	int64_t* slicedArray = slice(items, item_size, start, end);

	return mergesort(slicedArray, end-start, 0, end-start);
