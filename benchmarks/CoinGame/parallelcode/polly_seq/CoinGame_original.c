...
int64_t* _findMoves_(_DECL_1DARRAY_PARAM(moves), _DECL_DEALLOC_PARAM(moves), int64_t n, _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	while(true){
		...
		while(true){
			...
			//int64_t _17 = 1;
			//sub %18 = %16, %17 : int
			//int64_t _18=(i+1)*n+j-1;
			//indexof %19 = %0, %18 : int[]
			//int64_t _19=moves[(i+1)*n+j-1];
			//assign %6 = %19  : int
			// isCopyEliminated = true
