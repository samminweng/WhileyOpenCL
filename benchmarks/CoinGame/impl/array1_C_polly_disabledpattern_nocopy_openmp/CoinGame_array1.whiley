...
function findMoves(int[] moves, int n, int[] moves_out) -> int[]:
	// An array to store results at each step
	int[] moves_out = [0;n*n]
	int s = 0
	while s < n: // i0: s => 0<=s < n
		moves_out = moves// This assignment breaks parallelism
		int j = s
...
method main(System.Console sys):
	...
	if max != null:
		...
		moves = findMoves(moves, n, moves_out)
		//play(sys, moves, n)
