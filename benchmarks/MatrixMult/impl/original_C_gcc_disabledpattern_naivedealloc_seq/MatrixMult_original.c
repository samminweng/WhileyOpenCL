...
int main(int argc, char** args){
	...
	//assign %5 = %19  : int[]
	_ADD_DEALLOC(A, _19, int64_t);
	//invoke (%20) = (%5, %3, %4) MatrixMult_original:init : function(int[],int,int)->(int[])
	{
		_FUNCTIONCALL_COPY_PRE(_20, A, 0, "liveness: A = false, readonly: A = false, return:A = ALWAYS_RETURN" , "init");
		_DEALLOC(_20);
		void* tmp_data;
	...
	//assign %6 = %23  : int[]
	_ADD_DEALLOC(B, _23, int64_t);
	//invoke (%24) = (%6, %3, %4) MatrixMult_original:init : function(int[],int,int)->(int[])
	{
		_FUNCTIONCALL_COPY_PRE(_24, B, 0, "liveness: B = false, readonly: B = false, return:B = ALWAYS_RETURN" , "init");
		_DEALLOC(_24);
		void* tmp_data;
	...
	{
		_FUNCTIONCALL_COPY_PRE(_28, C, 2, "liveness: C = false, readonly: C = false, return:C = ALWAYS_RETURN" , "mat_mult");
		_FUNCTIONCALL_COPY_PRE(_28, B, 1, "liveness: B = false, readonly: B = true, return:B = NEVER_RETURN" , "mat_mult");
		_FUNCTIONCALL_COPY_PRE(_28, A, 0, "liveness: A = false, readonly: A = true, return:A = NEVER_RETURN" , "mat_mult");
		_DEALLOC(_28);
		void* tmp_a;
