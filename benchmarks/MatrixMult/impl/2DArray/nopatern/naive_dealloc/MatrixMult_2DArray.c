...
Matrix* matrix(int64_t width, int64_t height, _DECL_2DARRAY_PARAM(data), _DECL_DEALLOC_PARAM(data)){
	...
	_COPY_2DARRAY_int64_t(_4->data, data);
	_4->height = height;
	_4->width = width;
	_ADD_DEALLOC(_4);
	//return %4
...
Matrix* multiply(Matrix* A, _DECL_DEALLOC_PARAM(A), Matrix* B, _DECL_DEALLOC_PARAM(B)){
	...
	_COPY_2DARRAY_int64_t(C_data, _12);
	_ADD_DEALLOC(C_data);
	//const %13 = 0 : int
	_13 = 0;
	//assign %4 = %13  : int
...
blklab13:;
	...
	//invoke (%34) = (%35, %36, %3) MatrixMult_2DArray:matrix : function(MatrixMult_2DArray:nat,MatrixMult_2DArray:nat,int[][])->(MatrixMult_2DArray:Matrix)
	{
		...
		_34 = matrix(_35, _36, _COPY_2DARRAY_PARAM_int64_t(C_data), true);
...
blklab23:;
	//invoke (%15) = (%1, %0, %3) MatrixMult_2DArray:matrix : function(MatrixMult_2DArray:nat,MatrixMult_2DArray:nat,int[][])->(MatrixMult_2DArray:Matrix)
	{
		...
		_15 = matrix(width, height, _COPY_2DARRAY_PARAM_int64_t(rows), true);
...
int main(int argc, char** args){
	...
	//invoke (%17) = (%3, %4) MatrixMult_2DArray:multiply : function(MatrixMult_2DArray:Matrix,MatrixMult_2DArray:Matrix)->(MatrixMult_2DArray:Matrix)
	{
		...
		_17 = multiply(_COPY_STRUCT_PARAM(A, Matrix), true, _COPY_STRUCT_PARAM(B, Matrix), true);
