...
int main(int argc, char** args){
	...
	while(true){
		...
		//invoke (%28) = (%8, %4, %5) MatrixMult_original:init : function(int[],int,int)->(int[])
		{
			...
			_28 = _init_(B, B_size, false, width, height, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_28));
			_RESET_DEALLOC(B, "true-true-false" , "init");
			_RESET_DEALLOC_POST(_28, B);
		}
		//assign %8 = %28  : int[]
		_DEALLOC(B);
		// isCopyEliminated = true
		_UPDATE_1DARRAY(B, _28);
		_TRANSFER_DEALLOC(B, _28);
		//const %29 = 0 : int
		_29 = 0;
		//mul %30 = %4, %5 : int
		_30=width*height;
		...
		//invoke (%32) = (%7, %8, %9, %4, %5) MatrixMult_original:mat_mult : function(int[],int[],int[],int,int)->(int[])
		{
			...
			_32 = _mat_mult_(A, A_size, false, B, B_size, false, C, C_size, false, width, height, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_32));
			_RETAIN_DEALLOC(A, "false-false-false" , "mat_mult");
			_RETAIN_DEALLOC_POST(_32, A);
			_RETAIN_DEALLOC(B, "false-false-false" , "mat_mult");
			_RETAIN_DEALLOC_POST(_32, B);
			_RESET_DEALLOC(C, "true-true-false" , "mat_mult");
			_RESET_DEALLOC_POST(_32, C);
		}
		//assign %9 = %32  : int[]
		_DEALLOC(C);
		// isCopyEliminated = true
		_UPDATE_1DARRAY(C, _32);
		_TRANSFER_DEALLOC(C, _32);
