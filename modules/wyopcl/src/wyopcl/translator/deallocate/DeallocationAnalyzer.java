...
public class DeallocationAnalyzer extends Analyzer {
	 ...
	 * @return
	 */
	public List<String> computeOwnership(boolean isCopyEliminated, Code code, FunctionOrMethod function,CodeStores stores) {
		String indent = stores.getIndent(function);
		List<String> statements = new ArrayList<String>();
	 ...
	 * 
	 * If the parameter ownership is transferred to the calling function, 
	 * then its ownership is removed. For example, 
	 * <pre><code>
	 ...
	 * @return
	 */
	public List<String> computeOwnership(Codes.Invoke code, FunctionOrMethod function, CodeStores stores, Optional<CopyEliminationAnalyzer> copyAnalyzer) {
		List<String> statements = new ArrayList<String>();
		
		String indent = stores.getIndent(function);
		// Add or transfer out the parameters that do not have the copy
		for(int register: code.operands()){
			Optional<HashMap<String, Boolean>> ownership = computeCallParameterOwnership(register, code, function, stores,
					copyAnalyzer);
