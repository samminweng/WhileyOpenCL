...
public class DeallocationAnalyzer extends Analyzer {
	 ...
	 *
	 */
	public List<String> freeAllMemory(Bytecode.Return code, FunctionOrMethod function, CodeStores stores) {
	 ...
	 * @return
	 */
	public List<String> computeOwnership(boolean isCopyEliminated, Bytecode code, FunctionOrMethod function,
			CodeStores stores) {
		String indent = stores.getIndent(function);
		List<String> statements = new ArrayList<String>();
		if (code instanceof Operator) {
			...
			if (op.kind() == OperatorKind.ASSIGN) {
				...
				if (op.type(0) instanceof Type.Null) {
				...
				statements.add(indent + removeOwnership(lhs, function, stores));
			} else if(op.kind() == OperatorKind.RECORDCONSTRUCTOR){
				if (isCopyEliminated) {
					statements.add(indent + transferOwnership(lhs, rhs, function, stores));
					statements.add(indent + removeOwnership(rhs, function, stores));
				} else {
			...
			}else if(op.kind() == OperatorKind.ARRAYGENERATOR){
			...
			FieldLoad fieldload = (FieldLoad) code;
			int lhs = fieldload.target(0);
			if (fieldload.fieldName().equals("args")) {
			...
			Const constant = (Const) code;
			int lhs = constant.target(0);
			// Define NULL as a constant
			if (constant.constant() instanceof Constant.Null) {
	 ...
	 * @return
	 */
	public List<String> computeOwnership(Invoke code, FunctionOrMethod function, CodeStores stores,
	 ...
	 */
	public Optional<HashMap<String, Boolean>> computeCallParameterOwnership(int register, Invoke code,
	...
	public String addDeallocatedCode(String lhs, Update code, FunctionOrMethod function, CodeStores stores) {
		String indent = stores.getIndent(function);
