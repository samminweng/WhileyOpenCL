...
Image* sobelEdgeDetection(Image* input){
	...
	while(true){
		...
		while(true){
			...
			{
				_54 = convolution(_STRUCT_PARAM(input), x, y, _1DARRAY_PARAM(v_sobel));
			}
			//assign %11 = %54  : int
			v_g = _54;
			//invoke (%55) = (%0, %8, %9, %7) SobelEdge3:convolution : function(SobelEdge3:Image,int,int,int[])->(int)
			{
				_55 = convolution(_STRUCT_PARAM(input), x, y, _1DARRAY_PARAM(h_sobel));
			}
			//assign %12 = %55  : int
			h_g = _55;
...
blklab4:;
	...
	{
		_66 = image(width, height, _1DARRAY_PARAM(newPixels));
	}
	//return %66
	return _66;
	//return
