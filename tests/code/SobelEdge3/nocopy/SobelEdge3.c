...
Image* sobelEdgeDetection(Image* input){
	...
	while(true){
		...
		while(true){
			...
			{
				_51 = convolution(_STRUCT_PARAM(input), x, y, _1DARRAY_PARAM(v_sobel));
			}
			//assign %11 = %51  : int
			v_g = _51;
			//invoke (%52) = (%0, %8, %9, %7) SobelEdge3:convolution : function(SobelEdge3:Image,int,int,int[])->(int)
			{
				_52 = convolution(_STRUCT_PARAM(input), x, y, _1DARRAY_PARAM(h_sobel));
			}
			//assign %12 = %52  : int
			h_g = _52;
...
blklab4:;
	...
	{
		_63 = image(width, height, _1DARRAY_PARAM(newPixels));
	}
	//return %63
	return _63;
	//return
...
int main(int argc, char** args){
	...
	{
		write_image(stdout, _STRUCT_PARAM(output));
	}
	//assert
	{
