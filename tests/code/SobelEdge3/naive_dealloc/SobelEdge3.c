	...
	while(true){
		...
		while(true){
			...
			//invoke (%51) = (%0, %8, %9, %6) SobelEdge3:convolution : function(SobelEdge3:Image,int,int,int[])->(int)
			{
				void* input_tmp;
				void* v_sobel_tmp;
				_51 = convolution(_COPY_STRUCT_PARAM(input, Image), true, x, y, _COPY_1DARRAY_PARAM_int64_t(v_sobel), true);
			...
			//invoke (%52) = (%0, %8, %9, %7) SobelEdge3:convolution : function(SobelEdge3:Image,int,int,int[])->(int)
			{
				void* input_tmp;
				void* h_sobel_tmp;
				_52 = convolution(_COPY_STRUCT_PARAM(input, Image), true, x, y, _COPY_1DARRAY_PARAM_int64_t(h_sobel), true);
				_CALLEE_DEALLOC(input, "false-false-true" , "convolution");
...
blklab4:;
	//invoke (%63) = (%3, %4, %5) SobelEdge3:image : function(int,int,byte[])->(SobelEdge3:Image)
	{
		...
		_63 = image(width, height, _COPY_1DARRAY_PARAM_BYTE(newPixels), true);
