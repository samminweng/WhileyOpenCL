	...
	while(true){
		...
		while(true){
			...
			//invoke (%51) = (%0, %8, %9, %6) SobelEdge3:convolution : function(SobelEdge3:Image,int,int,int[])->(int)
			{
				void* input_tmp;
				void* v_sobel_tmp;
				_51 = convolution(_COPY_STRUCT_PARAM(input, Image), x, y, _COPY_1DARRAY_PARAM_int64_t(v_sobel));
			}
			//assign %11 = %51  : int
			v_g = _51;
			//invoke (%52) = (%0, %8, %9, %7) SobelEdge3:convolution : function(SobelEdge3:Image,int,int,int[])->(int)
			{
				void* input_tmp;
				void* h_sobel_tmp;
				_52 = convolution(_COPY_STRUCT_PARAM(input, Image), x, y, _COPY_1DARRAY_PARAM_int64_t(h_sobel));
			}
			//assign %12 = %52  : int
			h_g = _52;
...
blklab4:;
	...
	{
		void* newPixels_tmp;
		_63 = image(width, height, _COPY_1DARRAY_PARAM_BYTE(newPixels));
	}
	//return %63
	return _63;
	//return
...
void write_image(FILE* sys, Image* im){
	_DECL_1DARRAY_BYTE(pixels);
	int64_t width = 0;
	int64_t height = 0;
	int64_t size = 0;
...
int main(int argc, char** args){
	...
	{
		void* output_tmp;
		write_image(stdout, _COPY_STRUCT_PARAM(output, Image));
	}
	//assert
	{
