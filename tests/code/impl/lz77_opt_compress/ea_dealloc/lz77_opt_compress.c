...
BYTE* _compress_(BYTE* data, size_t data_size, _DECL_DEALLOC_PARAM(data), _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	_NEW1DARRAY_DEALLOC(_13, _12, arr_capacity, BYTE);
	//assign %1 = %13  : byte[]
	_ADD_DEALLOC(output, _13, BYTE);
	//const %14 = 0 : int
	_14 = 0;
	//assign %4 = %14  : int
	arr_size = _14;
	...
	while(true){
		...
		{
			_CALLEE_DEALLOC(_16, data, "false-false-true" , "findLongestMatch");
			void* tmp_data;
			_COPY_1DARRAY_PARAM(data, tmp_data, BYTE);
