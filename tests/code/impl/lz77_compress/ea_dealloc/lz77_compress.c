...
Match* _findLongestMatch_(BYTE* data, size_t data_size, _DECL_DEALLOC_PARAM(data), int64_t pos){
	...
	while(true){
		...
		{
			_CALLEE_DEALLOC(_14, data, "false-false-true" , "match");
			void* tmp_data;
...
BYTE* _compress_(BYTE* data, size_t data_size, _DECL_DEALLOC_PARAM(data), _DECL_1DARRAYSIZE_PARAM_CALLBYREFERENCE){
	...
	_8 = 0;
	//arraygen %9 = [7; 8] : byte[]
	_NEW1DARRAY_DEALLOC(_9, _7, _8, BYTE);
	//assign %1 = %9  : byte[]
	_ADD_DEALLOC(output, _9, BYTE);
	...
	while(true){
		...
		{
			_CALLEE_DEALLOC(_11, data, "false-false-true" , "findLongestMatch");
			void* tmp_data;
			_COPY_1DARRAY_PARAM(data, tmp_data, BYTE);
...
blklab17:;
		...
		{
			_CALLEE_DEALLOC(_22, output, "false-false-false" , "append");
			_DEALLOC(_22);
			void* tmp_items;
			_COPY_1DARRAY_PARAM(output, tmp_items, BYTE);
			_22 = _append_(tmp_items, output_size, true, offset, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_22));
			_CALLEE_DEALLOC_POST(_22, output);
		}
		//assign %1 = %22  : byte[]
		_ADD_DEALLOC(output, _22, BYTE);
		//invoke (%23) = (%1, %5) lz77_compress:append : function(byte[],byte)->(byte[])
		{
			...
			void* tmp_items;
			_COPY_1DARRAY_PARAM(output, tmp_items, BYTE);
			_23 = _append_(tmp_items, output_size, true, length, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_23));
...
int main(int argc, char** args){
	...
	//const %34 = [46,46,47,46,46,47,46,46,47,73,110,112,117,116,102,105,108,101,115,47,115,109,97,108,108,46,100,97,116] : int[]
	_DEALLOC(_34);
