...
Match* findLongestMatch(Data* input, _DECL_DEALLOC_PARAM(input), long long pos){
	...
	_DEALLOC(_9);
	_COPY_1DARRAY_BYTE(_9, input->items);
	_ADD_DEALLOC(_9);
	//assign %3 = %9  : byte[]
	_DEALLOC(data);
	...
	while(true){
		...
		{
			void* data_tmp;
			_16 = match(data_tmp = _COPY_1DARRAY_BYTE_PARAM(data), true, offset, pos);
...
Data* append_byte(Data* data, _DECL_DEALLOC_PARAM(data), BYTE item){
	...
	_DEALLOC(_7);
	_COPY_1DARRAY_BYTE(_7, data->items);
	_ADD_DEALLOC(_7);
	//assign %3 = %7  : byte[]
	_DEALLOC(items);
...
Data* compress(Data* input, _DECL_DEALLOC_PARAM(input)){
	...
	while(true){
		...
		_DEALLOC(_20);
		_COPY_1DARRAY_BYTE(_20, input->items);
		_ADD_DEALLOC(_20);
