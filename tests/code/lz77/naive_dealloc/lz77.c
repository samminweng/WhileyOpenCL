...
BYTE* write_u1(BYTE* bytes, long long bytes_size, _DECL_DEALLOC_PARAM(bytes), long long u1){
	...
	//invoke (%3) = (%0, %4) lz77:append_byte : function(byte[],byte)->(byte[])
	{
		...
		_DEALLOC(_3);
		_3 = append_byte(bytes_tmp = _COPY_1DARRAY_BYTE_PARAM(bytes), true, _4);
		_CALLEE_DEALLOC(bytes, "false-false-false");
		_ADD_DEALLOC(_3);
	}
	//return %3
	_DEALLOC(bytes);
...
Data* compress(Data* data, _DECL_DEALLOC_PARAM(data)){
	...
	while(true){
		...
		//invoke (%16) = (%2, %3) lz77:findLongestMatch : function(byte[],lz77:nat)->(lz77:Match)
		{
			...
			_16 = findLongestMatch(input_tmp = _COPY_1DARRAY_BYTE_PARAM(input), true, pos);
		...
		//invoke (%19) = (%4, %7) lz77:write_u1 : function(byte[],int)->(byte[])
		{
			...
			_DEALLOC(_19);
			_19 = write_u1(output_tmp = _COPY_1DARRAY_BYTE_PARAM(output), true, offset);
		...
		//invoke (%23) = (%4, %24) lz77:append_byte : function(byte[],byte)->(byte[])
		{
			...
			_DEALLOC(_23);
			_23 = append_byte(output_tmp = _COPY_1DARRAY_BYTE_PARAM(output), true, _24);
			_CALLEE_DEALLOC(output, "false-false-false");
			_ADD_DEALLOC(_23);
		}
		//assign %4 = %23  : byte[]
...
blklab10:;
		//invoke (%29) = (%4, %8) lz77:write_u1 : function(byte[],int)->(byte[])
		{
			...
			_DEALLOC(_29);
			_29 = write_u1(output_tmp = _COPY_1DARRAY_BYTE_PARAM(output), true, len);
			_CALLEE_DEALLOC(output, "false-false-false");
			_ADD_DEALLOC(_29);
		}
		//assign %4 = %29  : byte[]
...
int main(int argc, char** args){
	...
	//invoke (%20) = (%1) lz77:compress : function(lz77:Data)->(lz77:Data)
	{
		...
		_20 = compress(data_tmp = _COPY_STRUCT_PARAM(data, Data), true);
