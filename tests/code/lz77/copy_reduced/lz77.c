...
Match* findLongestMatch(Data* input, long long pos){
	...
	while(true){
		...
		{
			_14 = match(_STRUCT_PARAM(input), offset, pos);
		}
		//assign %7 = %14  : int
		len = _14;
...
Data* write_u1(Data* data, long long u1){
	...
	{
		_3 = append_byte(_STRUCT_PARAM(data), _4);
	}
	//return %3
	return _3;
	//return
...
Data* compress(Data* input){
	...
	while(true){
		...
		{
			_14 = findLongestMatch(_STRUCT_PARAM(input), pos);
		}
		...
		{
			_17 = write_u1(_STRUCT_PARAM(output), offset);
		}
		...
		{
			_19 = append_byte(_STRUCT_PARAM(output), _21);
		}
...
blklab10:;
		...
		{
			_24 = write_u1(_STRUCT_PARAM(output), len);
		}
...
Data* decompress(Data* input){
	...
	while(true){
		...
		{
			_28 = append_byte(_STRUCT_PARAM(output), item);
		}
...
blklab14:;
		...
		while(true){
			...
			{
				_37 = append_byte(_STRUCT_PARAM(output), item);
			}
...
int main(int argc, char** args){
	...
	{
		_19 = compress(_STRUCT_PARAM(data));
	}
	...
	_NEW_1DARRAY_LONGLONG(_22, 19, 0);
	...
	{
		_33 = decompress(_STRUCT_PARAM(compress_data));
	}
	...
	_NEW_1DARRAY_LONGLONG(_36, 16, 0);
