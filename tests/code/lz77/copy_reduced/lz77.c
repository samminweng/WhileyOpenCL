...
BYTE* write_u1(BYTE* bytes, long long bytes_size, long long u1){
	...
	//invoke (%3) = (%0, %4) lz77:append_byte : function(byte[],byte)->(byte[])
	{
		...
		_3 = append_byte(_1DARRAY_PARAM(bytes), _4);
	}
	//return %3
	return _3;
	//return
...
Data* compress(Data* data){
	...
	while(true){
		...
		{
			_16 = findLongestMatch(_1DARRAY_PARAM(input), pos);
		}
		...
		//invoke (%19) = (%4, %7) lz77:write_u1 : function(byte[],int)->(byte[])
		{
			...
			_19 = write_u1(_1DARRAY_PARAM(output), offset);
		}
		//assign %4 = %19  : byte[]
		...
		//invoke (%23) = (%4, %24) lz77:append_byte : function(byte[],byte)->(byte[])
		{
			...
			_23 = append_byte(_1DARRAY_PARAM(output), _24);
		}
		//assign %4 = %23  : byte[]
...
blklab10:;
		//invoke (%29) = (%4, %8) lz77:write_u1 : function(byte[],int)->(byte[])
		{
			...
			_29 = write_u1(_1DARRAY_PARAM(output), len);
		}
		//assign %4 = %29  : byte[]
