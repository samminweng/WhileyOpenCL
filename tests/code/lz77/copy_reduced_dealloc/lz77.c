...
Match* findLongestMatch(Data* input, _DECL_DEALLOC_PARAM(input), long long pos){
	...
	while(true){
		...
		//invoke (%14) = (%0, %6, %1) lz77:match : function(lz77:Data,lz77:nat,lz77:nat)->(int)
		{
			...
			_RETAIN_DEALLOC(input, "false-false-true" , "findLongestMatch");
		}
		//assign %7 = %14  : int
		len = _14;
...
Data* write_u1(Data* data, _DECL_DEALLOC_PARAM(data), long long u1){
	...
	//invoke (%3) = (%0, %4) lz77:append_byte : function(lz77:Data,byte)->(lz77:Data)
	{
		...
		_RETAIN_DEALLOC(data, "false-false-false" , "write_u1");
		_ADD_DEALLOC(_3);
	}
	//return %3
	_DEALLOC_STRUCT(data, Data);
...
Data* compress(Data* input, _DECL_DEALLOC_PARAM(input)){
	...
	while(true){
		...
		//invoke (%14) = (%0, %2) lz77:findLongestMatch : function(lz77:Data,lz77:nat)->(lz77:Match)
		{
			...
			_RETAIN_DEALLOC(input, "false-false-true" , "compress");
			_ADD_DEALLOC(_14);
		}
		...
		//invoke (%17) = (%3, %5) lz77:write_u1 : function(lz77:Data,int)->(lz77:Data)
		{
			...
			_RETAIN_DEALLOC(output, "false-false-false" , "compress");
			_ADD_DEALLOC(_17);
		}
...
Data* decompress(Data* input, _DECL_DEALLOC_PARAM(input)){
	...
	while(true){
		...
		//invoke (%28) = (%2, %6) lz77:append_byte : function(lz77:Data,byte)->(lz77:Data)
		{
			...
			_RETAIN_DEALLOC(output, "false-false-false" , "decompress");
			_ADD_DEALLOC(_28);
		}
...
Data* init(long long repeat){
	...
	//assign %3 = %8  : byte[]
	_DEALLOC(items);
	_UPDATE_1DARRAY(items, _8);
	_TRANSFER_DEALLOC(items, _8);
	//const %9 = 0 : int
	_9 = 0;
	//assign %4 = %9  : int
	pos = _9;
...
blklab18:;
	...
	_UPDATE_1DARRAY(_16->items, items);
	_16->length = length;
...
int main(int argc, char** args){
	...
	{
		_DEALLOC_STRUCT(_19, Data);
		_19 = compress(_STRUCT_PARAM(data), false);
		_RETAIN_DEALLOC(data, "false-false-false" , "main");
		_ADD_DEALLOC(_19);
	}
	...
	//invoke (%33) = (%2) lz77:decompress : function(lz77:Data)->(lz77:Data)
	{
		...
		_33 = decompress(_STRUCT_PARAM(compress_data), false);
		_RETAIN_DEALLOC(compress_data, "false-false-false" , "main");
		_ADD_DEALLOC(_33);
	}
