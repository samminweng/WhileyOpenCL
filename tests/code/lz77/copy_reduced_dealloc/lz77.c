...
BYTE* append_byte(BYTE* items, long long items_size, _DECL_DEALLOC_PARAM(items), BYTE item){
	...
	//assign %3 = %9  : byte[]
	_DEALLOC(nitems);
	_UPDATE_1DARRAY(nitems, _9);
	_TRANSFER_DEALLOC(nitems, _9);
	//const %10 = 0 : int
	_10 = 0;
	//assign %4 = %10  : int
	i = _10;
...
BYTE* write_u1(BYTE* bytes, long long bytes_size, _DECL_DEALLOC_PARAM(bytes), long long u1){
	_DECL_1DARRAY_BYTE(_2);
	_DECL_DEALLOC(_2);
	...
	//invoke (%3) = (%0, %4) lz77:append_byte : function(byte[],byte)->(byte[])
	{
		...
		_DEALLOC(_3);
		_3 = append_byte(_1DARRAY_PARAM(bytes), false, _4);
		_RETAIN_DEALLOC(bytes, "false-false-false");
		_ADD_DEALLOC(_3);
	}
	//return %3
	_DEALLOC(bytes);
...
Data* compress(Data* data, _DECL_DEALLOC_PARAM(data)){
	Data* _1;
	_DECL_DEALLOC(_1);
	...
	_DEALLOC(_9);
	_UPDATE_1DARRAY(_9, data->items);
	_REMOVE_DEALLOC(_9);
	//assign %2 = %9  : byte[]
	_DEALLOC(input);
	_UPDATE_1DARRAY(input, _9);
	...
	while(true){
		...
		//invoke (%16) = (%2, %3) lz77:findLongestMatch : function(byte[],lz77:nat)->(lz77:Match)
		{
			...
			_16 = findLongestMatch(_1DARRAY_PARAM(input), false, pos);
		...
		//invoke (%19) = (%4, %7) lz77:write_u1 : function(byte[],int)->(byte[])
		{
			...
			_DEALLOC(_19);
			_19 = write_u1(_1DARRAY_PARAM(output), false, offset);
			_RETAIN_DEALLOC(output, "false-false-false");
			_ADD_DEALLOC(_19);
		}
		//assign %4 = %19  : byte[]
		_DEALLOC(output);
		_UPDATE_1DARRAY(output, _19);
		...
		//invoke (%23) = (%4, %24) lz77:append_byte : function(byte[],byte)->(byte[])
		{
			...
			_DEALLOC(_23);
			_23 = append_byte(_1DARRAY_PARAM(output), false, _24);
			_RETAIN_DEALLOC(output, "false-false-false");
			_ADD_DEALLOC(_23);
		}
		//assign %4 = %23  : byte[]
...
blklab10:;
		//invoke (%29) = (%4, %8) lz77:write_u1 : function(byte[],int)->(byte[])
		{
			...
			_DEALLOC(_29);
			_29 = write_u1(_1DARRAY_PARAM(output), false, len);
			_RETAIN_DEALLOC(output, "false-false-false");
			_ADD_DEALLOC(_29);
		}
		//assign %4 = %29  : byte[]
		_DEALLOC(output);
		_UPDATE_1DARRAY(output, _29);
		_TRANSFER_DEALLOC(output, _29);
		//const %30 = 1 : int
		_30 = 1;
		//add %31 = %5, %30 : int
		_31=length+_30;
...
blklab8:;
	...
	_UPDATE_1DARRAY(_35->items, output);
	_35->length = length;
	_REMOVE_DEALLOC(output);
	_ADD_DEALLOC(_35);
	//return %35
