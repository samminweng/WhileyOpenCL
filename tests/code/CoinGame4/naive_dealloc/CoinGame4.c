#include "CoinGame4.h"
void play(FILE* sys, _DECL_1DARRAY_PARAM(moves), _DECL_DEALLOC_PARAM(moves), long long n){
	long long left = 0;
	long long right = 0;
	long long i = 0;
...
blklab17:;
			//invoke (%41) = (%0, %5, %4, %1, %7, %6, %8) CoinGame4:pickCoin : function(int[],int,int,int,int,int,int)->(int[])
			{
				...
				_DEALLOC(_41);
				_41 = pickCoin(moves_tmp = _COPY_1DARRAY_PARAM(moves), false, i, j, n, x, y, z);
