...
blklab13:;
	...
	//invoke (%34) = (%35, %36, %3) MatrixMult:matrix : function(MatrixMult:nat,MatrixMult:nat,int[][])->(MatrixMult:Matrix)
	{
		...
		_34 = matrix(_35, _36, C_data_tmp = _COPY_2DARRAY_PARAM(C_data, T_INT), true);
		_CALLEE_DEALLOC(C_data, "false-false-false" , "multiply");
		_ADD_DEALLOC(_34);
	}
	//return %34
...
blklab23:;
	//invoke (%15) = (%1, %0, %3) MatrixMult:matrix : function(MatrixMult:nat,MatrixMult:nat,int[][])->(MatrixMult:Matrix)
	{
		...
		_15 = matrix(width, height, rows_tmp = _COPY_2DARRAY_PARAM(rows, T_INT), true);
		_CALLEE_DEALLOC(rows, "false-false-false" , "init");
		_ADD_DEALLOC(_15);
	}
	//return %15
	_DEALLOC_STRUCT(r, Matrix);
...
blklab29:;
	...
	//invoke () = (%0, %4) MatrixMult:printMat : method(whiley/lang/System:Console,MatrixMult:Matrix)->()
	{
		...
		_CALLEE_DEALLOC(C, "false-false-false" , "main");
	}
