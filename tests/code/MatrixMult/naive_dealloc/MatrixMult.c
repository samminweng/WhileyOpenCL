...
void printf_Matrix(Matrix* matrix){
	printf("{");
	printf(" data:");
	_PRINT_2DARRAY(matrix->data, T_INT);
	printf(" width:");
...
Matrix* matrix(long long width, long long height, _DECL_2DARRAY_PARAM(data), _DECL_DEALLOC_PARAM(data)){
	...
	_COPY_2DARRAY_LONGLONG(_4->data, data);
	_4->height = height;
	_4->width = width;
	_ADD_DEALLOC(_4);
	//return %4
	_DEALLOC_2DArray(data, T_INT);
...
Matrix* multiply(Matrix* A, _DECL_DEALLOC_PARAM(A), Matrix* B, _DECL_DEALLOC_PARAM(B)){
	...
	_DEALLOC_2DArray(_12, T_INT);
	_NEW_2DARRAY_LONGLONG(_12, _11, _10);
	_ADD_DEALLOC(_12);
	//assign %3 = %12  : int[][]
	_DEALLOC_2DArray(C_data, T_INT);
	_COPY_2DARRAY_LONGLONG(C_data, _12);
	_ADD_DEALLOC(C_data);
	//const %13 = 0 : int
	_13 = 0;
	//assign %4 = %13  : int
...
blklab13:;
	...
	//invoke (%34) = (%35, %36, %3) MatrixMult:matrix : function(MatrixMult:nat,MatrixMult:nat,int[][])->(MatrixMult:Matrix)
	{
		...
		_34 = matrix(_35, _36, _COPY_2DARRAY_PARAM_LONGLONG(C_data), true);
...
Matrix* init(long long height, long long width){
	...
	_ADD_DEALLOC(_8);
	//assign %3 = %8  : int[][]
	_DEALLOC_2DArray(rows, T_INT);
	_COPY_2DARRAY_LONGLONG(rows, _8);
	_ADD_DEALLOC(rows);
	//const %9 = 0 : int
	_9 = 0;
	//assign %4 = %9  : int
...
blklab23:;
	//invoke (%15) = (%1, %0, %3) MatrixMult:matrix : function(MatrixMult:nat,MatrixMult:nat,int[][])->(MatrixMult:Matrix)
	{
		...
		_15 = matrix(width, height, _COPY_2DARRAY_PARAM_LONGLONG(rows), true);
...
blklab28:;
	...
	//invoke (%26) = (%2, %3) MatrixMult:multiply : function(MatrixMult:Matrix,MatrixMult:Matrix)->(MatrixMult:Matrix)
	{
		...
		_26 = multiply(_COPY_STRUCT_PARAM(A, Matrix), true, _COPY_STRUCT_PARAM(B, Matrix), true);
...
blklab29:;
	...
	_DEALLOC(_37);
	_NEW_1DARRAY_LONGLONG(_37, 25, 0);
