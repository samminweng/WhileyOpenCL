...
int main(int argc, char** args){
	...
	//invoke (%14) = (%4, %1, %2) SobelEdge1:sobelEdgeDetection : function(byte[],int,int)->(byte[])
	{
		void* pixels_tmp;
		_DEALLOC(_14);
		_14 = sobelEdgeDetection(_COPY_1DARRAY_PARAM_BYTE(pixels), true, width, height, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_14));
	...
	//invoke () = (%0, %5, %1, %2) SobelEdge1:printImage : method(whiley/lang/System:Console,byte[],int,int)->()
	{
		...
		_CALLEE_DEALLOC(newPixels, "false-false-true" , "printImage");
	}
