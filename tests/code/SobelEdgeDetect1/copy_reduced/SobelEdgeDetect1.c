...
int main(int argc, char** args){
	...
	while(true){
		...
		while(true){
			...
			_63 = convolution(_1DARRAY_PARAM(pixels), width, height, x, y, _1DARRAY_PARAM(v_sobel));
			//assign %11 = %63  : int
			v_g = _63;
			//invoke (%64) = (%4, %1, %2, %8, %9, %7) SobelEdgeDetect1:convolution : function(int[],int,int,int,int,int[])->(int)
			_64 = convolution(_1DARRAY_PARAM(pixels), width, height, x, y, _1DARRAY_PARAM(h_sobel));
			//assign %12 = %64  : int
			h_g = _64;
