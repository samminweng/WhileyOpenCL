...
int main(int argc, char** args){
	...
	while(true){
		...
		while(true){
			...
			_63 = convolution(_COPY_1DARRAY_PARAM(pixels), true, width, height, x, y, _COPY_1DARRAY_PARAM(v_sobel), true);
			_ADD_DEALLOC(pixels);
			_ADD_DEALLOC(v_sobel);
			//assign %11 = %63  : int
			v_g = _63;
			//invoke (%64) = (%4, %1, %2, %8, %9, %7) SobelEdgeDetect1:convolution : function(int[],int,int,int,int,int[])->(int)
			_64 = convolution(_COPY_1DARRAY_PARAM(pixels), true, width, height, x, y, _COPY_1DARRAY_PARAM(h_sobel), true);
			_ADD_DEALLOC(pixels);
			_ADD_DEALLOC(h_sobel);
			//assign %12 = %64  : int
			h_g = _64;
