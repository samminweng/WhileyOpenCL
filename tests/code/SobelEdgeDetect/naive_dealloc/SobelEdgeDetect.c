...
int main(int argc, char** args){
	...
	while(true){
		...
		while(true){
			...
			_53 = convolution(_COPY_1DARRAY_PARAM(pixels), true, width, height, x, y, _COPY_1DARRAY_PARAM(v_sobel), true);
			_ADD_DEALLOC(pixels);
			_ADD_DEALLOC(v_sobel);
			//assign %11 = %53  : int
			v_g = _53;
			//invoke (%54) = (%4, %1, %2, %8, %9, %7) SobelEdgeDetect:convolution : function(int[],int,int,int,int,int[])->(int)
			_54 = convolution(_COPY_1DARRAY_PARAM(pixels), true, width, height, x, y, _COPY_1DARRAY_PARAM(h_sobel), true);
			_ADD_DEALLOC(pixels);
			_ADD_DEALLOC(h_sobel);
			//assign %12 = %54  : int
			h_g = _54;
