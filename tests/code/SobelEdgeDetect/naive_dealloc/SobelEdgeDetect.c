	...
	while(true){
		...
		while(true){
			...
			//invoke (%47) = (%0, %1, %2, %8, %9, %6) SobelEdgeDetect:convolution : function(int[],int,int,int,int,int[])->(int)
			{
				void* pixels_tmp;
				void* v_sobel_tmp;
				_47 = convolution(_COPY_1DARRAY_PARAM_int64_t(pixels), true, width, height, x, y, _COPY_1DARRAY_PARAM_int64_t(v_sobel), true);
			...
			//invoke (%48) = (%0, %1, %2, %8, %9, %7) SobelEdgeDetect:convolution : function(int[],int,int,int,int,int[])->(int)
			{
				void* pixels_tmp;
				void* h_sobel_tmp;
				_48 = convolution(_COPY_1DARRAY_PARAM_int64_t(pixels), true, width, height, x, y, _COPY_1DARRAY_PARAM_int64_t(h_sobel), true);
				_CALLEE_DEALLOC(pixels, "false-false-true" , "convolution");
...
int main(int argc, char** args){
	...
	//assign %4 = %10  : int[]
	_DEALLOC(pixels);
	_COPY_1DARRAY_int64_t(pixels, _10);
	_ADD_DEALLOC(pixels);
	//const %11 = 0 : int
	_11 = 0;
	//const %12 = 4 : int
	...
	//invoke (%16) = (%4, %1, %2) SobelEdgeDetect:sobelEdgeDetection : function(int[],int,int)->(int[])
	{
		void* pixels_tmp;
		_DEALLOC(_16);
		_16 = sobelEdgeDetection(_COPY_1DARRAY_PARAM_int64_t(pixels), true, width, height, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_16));
	...
	{
		void* pixels_tmp;
		printImage(stdout, _COPY_1DARRAY_PARAM_int64_t(pixels), true, width, height);
	...
	{
		void* newPixels_tmp;
		printImage(stdout, _COPY_1DARRAY_PARAM_int64_t(newPixels), true, width, height);
