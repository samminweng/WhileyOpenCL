...
int main(int argc, char** args){
	...
	while(true){
		...
		while(true){
			...
			_59 = convolution(_COPY_1DARRAY_PARAM(pixels), true, width, height, x, y, _COPY_1DARRAY_PARAM(v_sobel), true);
			_ADD_DEALLOC(pixels);
			_ADD_DEALLOC(v_sobel);
			//assign %11 = %59  : int
			v_g = _59;
			//invoke (%60) = (%4, %1, %2, %8, %9, %7) SobelEdgeDetect:convolution : function(int[],int,int,int,int,int[])->(int)
			_60 = convolution(_COPY_1DARRAY_PARAM(pixels), true, width, height, x, y, _COPY_1DARRAY_PARAM(h_sobel), true);
			_ADD_DEALLOC(pixels);
			_ADD_DEALLOC(h_sobel);
			//assign %12 = %60  : int
			h_g = _60;
