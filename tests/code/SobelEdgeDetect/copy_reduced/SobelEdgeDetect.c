...
int main(int argc, char** args){
	...
	while(true){
		...
		while(true){
			...
			_59 = convolution(_1DARRAY_PARAM(pixels), width, height, x, y, _1DARRAY_PARAM(v_sobel));
			//assign %11 = %59  : int
			v_g = _59;
			//invoke (%60) = (%4, %1, %2, %8, %9, %7) SobelEdgeDetect:convolution : function(int[],int,int,int,int,int[])->(int)
			_60 = convolution(_1DARRAY_PARAM(pixels), width, height, x, y, _1DARRAY_PARAM(h_sobel));
			//assign %12 = %60  : int
			h_g = _60;
			//add %61 = %11, %12 : int
			_61=v_g+h_g;
