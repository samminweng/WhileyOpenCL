...
int main(int argc, char** args){
	...
	while(true){
		...
		while(true){
			...
			_66 = convolution(_1DARRAY_PARAM(pixels), !pixels_dealloc, width, height, x, y, _1DARRAY_PARAM(v_sobel), !v_sobel_dealloc);
			//assign %11 = %66  : int
			v_g = _66;
			//invoke (%67) = (%4, %1, %2, %8, %9, %7) SobelEdgeDetect:convolution : function(int[],int,int,int,int,int[])->(int)
			_67 = convolution(_1DARRAY_PARAM(pixels), !pixels_dealloc, width, height, x, y, _1DARRAY_PARAM(h_sobel), !h_sobel_dealloc);
			//assign %12 = %67  : int
			h_g = _67;
