...
#define _CALLER_DEALLOC(a, b) \
		({\
			DEBUG_PRINT("_CALLER_DEALLOC macro on "str(#b));\
			if(a != b##_tmp){\
...
#define _CALLEE_DEALLOC(b) DEBUG_PRINT("_CALLEE_DEALLOC macro on "str(#b));
...
#define _RETAIN_DEALLOC(b) DEBUG_PRINT("_RETAIN_DEALLOC macro on "str(#b));
...
#define _RESET_DEALLOC(b) \
		({\
			DEBUG_PRINT("_RESET_DEALLOC macro on "str(#b));\
			b##_dealloc = false;\
		})
/*
* Other Macros 
