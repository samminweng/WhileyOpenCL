...
int main(int argc, char** args){
	...
	_23 = matrix(width, height, _1DARRAY_PARAM(pixels), false);
	_REMOVE_DEALLOC(pixels);
	_ADD_DEALLOC(_23);
	//assign %5 = %23  : {int height,int[] pixels,int width}
	_DEALLOC_STRUCT(image, Matrix);
	image = _23;
	_TRANSFER_DEALLOC(image, _23);
	//const %24 = 255 : int
	_24 = 255;
	...
	_TRANSFER_DEALLOC(newPixels, _25);
	//const %26 = 1 : int
	_26 = 1;
	//neg %27 = %26 : int
	_27= -_26;
	...
	while(true){
		...
		while(true){
			...
			_56 = convolution(_STRUCT_PARAM(image), !image_dealloc, x, y, _1DARRAY_PARAM(v_sobel), !v_sobel_dealloc);
			//assign %12 = %56  : int
			v_g = _56;
			//invoke (%57) = (%5, %9, %10, %8) SobelEdgeDetect2:convolution : function(SobelEdgeDetect2:Matrix,int,int,int[])->(int)
			_57 = convolution(_STRUCT_PARAM(image), !image_dealloc, x, y, _1DARRAY_PARAM(h_sobel), !h_sobel_dealloc);
			//assign %13 = %57  : int
			h_g = _57;
...
print_mat(stdout, _STRUCT_PARAM(image), !image_dealloc);
...
print_mat(stdout, _STRUCT_PARAM(newImage), !newImage_dealloc);
	//return
	_DEALLOC(pixels);
	_DEALLOC_STRUCT(image, Matrix);
	_DEALLOC(newPixels);
