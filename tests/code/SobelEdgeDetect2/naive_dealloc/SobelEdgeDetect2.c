...
long long convolution(Matrix* a, _DECL_DEALLOC_PARAM(a), long long xCenter, long long yCenter, _DECL_1DARRAY_PARAM(filter), _DECL_DEALLOC_PARAM(filter)){
	...
	_DEALLOC(_21);
	_COPY_1DARRAY(_21, a->pixels);
	_ADD_DEALLOC(_21);
	//assign %9 = %21  : int[]
	_DEALLOC(pixels);
	_COPY_1DARRAY(pixels, _21);
	_ADD_DEALLOC(pixels);
...
int main(int argc, char** args){
	...
	_DEALLOC_STRUCT(_23, Matrix);
	_23 = matrix(width, height, _COPY_1DARRAY_PARAM(pixels), true);
	_ADD_DEALLOC(pixels);
	_ADD_DEALLOC(_23);
	//assign %5 = %23  : {int height,int[] pixels,int width}
	_DEALLOC_STRUCT(image, Matrix);
	image = copy_Matrix(_23);
	_ADD_DEALLOC(image);
	//const %24 = 255 : int
	_24 = 255;
	...
	while(true){
		...
		while(true){
			...
			_56 = convolution(_COPY_STRUCT_PARAM(image, Matrix), true, x, y, _COPY_1DARRAY_PARAM(v_sobel), true);
			_ADD_DEALLOC(image);
			_ADD_DEALLOC(v_sobel);
			//assign %12 = %56  : int
			v_g = _56;
			//invoke (%57) = (%5, %9, %10, %8) SobelEdgeDetect2:convolution : function(SobelEdgeDetect2:Matrix,int,int,int[])->(int)
			_57 = convolution(_COPY_STRUCT_PARAM(image, Matrix), true, x, y, _COPY_1DARRAY_PARAM(h_sobel), true);
			_ADD_DEALLOC(image);
			_ADD_DEALLOC(h_sobel);
			//assign %13 = %57  : int
			h_g = _57;
...
blklab11:;
	...
	_DEALLOC_STRUCT(_68, Matrix);
	_68 = matrix(width, height, _COPY_1DARRAY_PARAM(newPixels), true);
	_ADD_DEALLOC(newPixels);
	_ADD_DEALLOC(_68);
	//assign %15 = %68  : {int height,int[] pixels,int width}
	_DEALLOC_STRUCT(newImage, Matrix);
	newImage = copy_Matrix(_68);
	_ADD_DEALLOC(newImage);
...
print_mat(stdout, _COPY_STRUCT_PARAM(image, Matrix), true);
	_ADD_DEALLOC(image);
...
print_mat(stdout, _COPY_STRUCT_PARAM(newImage, Matrix), true);
	_ADD_DEALLOC(newImage);
	//return
	_DEALLOC(pixels);
	_DEALLOC_STRUCT(image, Matrix);
