...
Image* copy_Image(Image* _Image){
	...
	_COPY_1DARRAY_int64_t(new_Image->pixels, _Image->pixels);
...
Image* image(int64_t width, int64_t height, _DECL_1DARRAY_PARAM(pixels), _DECL_DEALLOC_PARAM(pixels)){
	...
	_4->height = height;
	_COPY_1DARRAY_int64_t(_4->pixels, pixels);
	_4->width = width;
	_ADD_DEALLOC(_4);
	//return %4
	_DEALLOC(pixels);
...
int64_t convolution(Image* A, _DECL_DEALLOC_PARAM(A), int64_t xCenter, int64_t yCenter, _DECL_1DARRAY_PARAM(filter), _DECL_DEALLOC_PARAM(filter)){
	...
	//assign %5 = %17  : int[]
	_DEALLOC(pixels);
	_COPY_1DARRAY_int64_t(pixels, _17);
	_ADD_DEALLOC(pixels);
...
Image* sobelEdgeDetection(Image* input, _DECL_DEALLOC_PARAM(input)){
	...
	//assign %5 = %19  : int[]
	_DEALLOC(newPixels);
	_COPY_1DARRAY_int64_t(newPixels, _19);
...
blklab4:;
	//invoke (%62) = (%3, %4, %5) SobelEdgeDetect2:image : function(int,int,int[])->(SobelEdgeDetect2:Image)
	{
		...
		_62 = image(width, height, _COPY_1DARRAY_PARAM_int64_t(newPixels), true);
...
void print_image(FILE* sys, Image* im, _DECL_DEALLOC_PARAM(im)){
	...
	//assign %2 = %8  : int[]
	_DEALLOC(pixels);
	_COPY_1DARRAY_int64_t(pixels, _8);
	_ADD_DEALLOC(pixels);
...
int main(int argc, char** args){
	...
	_DEALLOC(_11);
	_NEW_1DARRAY_int64_t(_11, size, _10);
	_ADD_DEALLOC(_11);
	//assign %4 = %11  : int[]
	_DEALLOC(pixels);
	_COPY_1DARRAY_int64_t(pixels, _11);
	_ADD_DEALLOC(pixels);
	//const %12 = 0 : int
	_12 = 0;
	//const %13 = 0 : int
	...
	//invoke (%19) = (%1, %2, %4) SobelEdgeDetect2:image : function(int,int,int[])->(SobelEdgeDetect2:Image)
	{
		...
		_19 = image(width, height, _COPY_1DARRAY_PARAM_int64_t(pixels), true);
