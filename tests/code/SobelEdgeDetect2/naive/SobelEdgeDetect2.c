...
int main(int argc, char** args){
	...
	_23 = matrix(width, height, _COPY_1DARRAY_PARAM(pixels));
	...
	while(true){
		...
		while(true){
			...
			_56 = convolution(_COPY_STRUCT_PARAM(image, Matrix), x, y, _COPY_1DARRAY_PARAM(v_sobel));
			//assign %12 = %56  : int
			v_g = _56;
			//invoke (%57) = (%5, %9, %10, %8) SobelEdgeDetect2:convolution : function(SobelEdgeDetect2:Matrix,int,int,int[])->(int)
			_57 = convolution(_COPY_STRUCT_PARAM(image, Matrix), x, y, _COPY_1DARRAY_PARAM(h_sobel));
			//assign %13 = %57  : int
			h_g = _57;
...
blklab11:;
	...
	_68 = matrix(width, height, _COPY_1DARRAY_PARAM(newPixels));
...
print_mat(stdout, _COPY_STRUCT_PARAM(image, Matrix));
...
print_mat(stdout, _COPY_STRUCT_PARAM(newImage, Matrix));
	//return
	exit(0);
}
