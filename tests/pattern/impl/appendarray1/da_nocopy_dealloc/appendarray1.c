...
blklab7:;
	//invoke (%25) = (%1, %20) appendarray1:resize : function(byte[],int)->(byte[])
	{
		...
		_25 = _resize_(output, output_size, false, _20, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_25));
		_RETAIN_DEALLOC(output, "false-false-false" , "resize");
		_RETAIN_DEALLOC_POST(_25, output);
	}
	//assign %1 = %25  : byte[]
	_DEALLOC(output);
	// isCopyEliminated = true
...
blklab9:;
	...
	{
		// isCopyEliminated of '_1' = true
		_25 = _resize_(output, output_size, false, size, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_25));
		_RETAIN_DEALLOC(output, "false-false-false" , "resize");
		_RETAIN_DEALLOC_POST(_25, output);
	}
	//assign %1 = %25  : byte[]
...
int main(int argc, char** args){
	...
	{
		// isCopyEliminated of '_1' = true
		_8 = _f_append_(input, input_size, false, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_8));
		_RETAIN_DEALLOC(input, "false-false-true" , "f_append");
		_RETAIN_DEALLOC_POST(_8, input);
	}
	//assign %2 = %8  : byte[]
	_DEALLOC(arr);
	// isCopyEliminated = true
	...
	{
		// isCopyEliminated of '_1' = true
		_9 = _f_resize_(input, input_size, false, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_9));
		_RETAIN_DEALLOC(input, "false-false-false" , "f_resize");
		_RETAIN_DEALLOC_POST(_9, input);
	}
	//assign %3 = %9  : byte[]
	_DEALLOC(arr1);
	// isCopyEliminated = true
