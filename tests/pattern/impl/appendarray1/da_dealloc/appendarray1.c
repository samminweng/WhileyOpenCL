...
blklab7:;
	//invoke (%25) = (%1, %20) appendarray1:resize : function(byte[],int)->(byte[])
	{
		...
		_DEALLOC(_25);
		_25 = _resize_(tmp_items, output_size, true, _20, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_25));
		_CALLEE_DEALLOC(output, "false-false-false" , "resize");
		_CALLEE_DEALLOC_POST(_25, output);
	}
	//assign %1 = %25  : byte[]
	_DEALLOC(output);
...
blklab9:;
	//invoke (%25) = (%1, %3) appendarray1:resize : function(byte[],int)->(byte[])
	{
		...
		_COPY_1DARRAY_PARAM(output, tmp_items, BYTE);
		_DEALLOC(_25);
		_25 = _resize_(tmp_items, output_size, true, size, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_25));
		_CALLEE_DEALLOC(output, "false-false-false" , "resize");
		_CALLEE_DEALLOC_POST(_25, output);
	}
	//assign %1 = %25  : byte[]
...
int main(int argc, char** args){
	...
	//invoke (%8) = (%1) appendarray1:f_append : function(byte[])->(byte[])
	{
		...
		_DEALLOC(_8);
		_8 = _f_append_(tmp_input, input_size, true, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_8));
		_CALLEE_DEALLOC(input, "false-false-true" , "f_append");
		_CALLEE_DEALLOC_POST(_8, input);
	}
	//assign %2 = %8  : byte[]
	_DEALLOC(arr);
	...
	//invoke (%9) = (%1) appendarray1:f_resize : function(byte[])->(byte[])
	{
		...
		_COPY_1DARRAY_PARAM(input, tmp_input, BYTE);
		_DEALLOC(_9);
		_9 = _f_resize_(tmp_input, input_size, true, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_9));
		_CALLEE_DEALLOC(input, "false-false-false" , "f_resize");
		_CALLEE_DEALLOC_POST(_9, input);
	}
	//assign %3 = %9  : byte[]
	_DEALLOC(arr1);
