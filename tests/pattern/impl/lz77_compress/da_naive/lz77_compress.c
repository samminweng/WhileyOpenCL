...
blklab12:;
	//invoke (%33) = (%1, %28) lz77_compress:resize : function(byte[],int)->(byte[])
	{
		...
		_33 = _resize_(tmp_items, output_size, _28, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_33));
	}
	//assign %1 = %33  : byte[]
...
int main(int argc, char** args){
	...
	//invoke (%22) = (%2) lz77_compress:compress : function(byte[])->(byte[])
	{
		void* tmp_data;
		_COPY_1DARRAY_PARAM(data, tmp_data, BYTE);
		_22 = _compress_(tmp_data, data_size, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_22));
	}
	//assign %3 = %22  : byte[]
	...
	//invoke (%37) = (%3) lz77_compress:decompress : function(byte[])->(byte[])
	{
		...
		_37 = _decompress_(tmp_data, compress_data_size, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_37));
	}
	//assign %5 = %37  : byte[]
