...
int main(int argc, char** args){
	...
	//invoke (%22) = (%2) lz77_compress:compress : function(byte[])->(byte[])
	{
		...
		_22 = _compress_(data, data_size, false, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_22));
		_RETAIN_DEALLOC(data, "false-false-false" , "compress");
		_RETAIN_DEALLOC_POST(_22, data);
	}
	//assign %3 = %22  : byte[]
	...
	//invoke (%37) = (%3) lz77_compress:decompress : function(byte[])->(byte[])
	{
		...
		_37 = _decompress_(compress_data, compress_data_size, false, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_37));
		_RETAIN_DEALLOC(compress_data, "false-false-false" , "decompress");
		_RETAIN_DEALLOC_POST(_37, compress_data);
	}
	//assign %5 = %37  : byte[]
	_DEALLOC(decompress_data);
	// isCopyEliminated = true
	_UPDATE_1DARRAY(decompress_data, _37);
	_TRANSFER_DEALLOC(decompress_data, _37);
