...
int main(int argc, char** args){
	...
	{
		_DEALLOC(_3);
		_3 = reverse(_1DARRAY_PARAM(_9), false, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_3));
		_RETAIN_DEALLOC(_9, "false-false-false" , "reverse");
		_ADD_DEALLOC(_3);
	}
	//assign %1 = %3  : int[]
	_DEALLOC(rs);
	_UPDATE_1DARRAY(rs, _3);
	_TRANSFER_DEALLOC(rs, _3);
	//assert
	{
		//const %10 = 5 : int
		_10 = 5;
		...
		_IFEQ_ARRAY_int64_t(rs, _15, blklab4);
		//fail
		fprintf(stderr,"fail");
		exit(-1);
//.blklab4
blklab4:;
	...
	{
		_DEALLOC(_18);
		_18 = reverse(_1DARRAY_PARAM(_29), false, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_18));
		_RETAIN_DEALLOC(_29, "false-false-false" , "reverse");
		_ADD_DEALLOC(_18);
	}
	//assign %2 = %18  : int[]
	_DEALLOC(rs2);
	_UPDATE_1DARRAY(rs2, _18);
	_TRANSFER_DEALLOC(rs2, _18);
