...
blklab0:;
	...
	_NEW1DARRAY_DEALLOC(_20, _18, _19, int64_t);
	//return %20
	_DEALLOC(a);
	_DEALLOC(b);
	_DEALLOC(c);
	_DEALLOC(_3);
	_DEALLOC(_17);
	_UPDATE_1DARRAYSZIE_PARAM_CALLBYREFERENCE(_20);
	return _20;
//.blklab1
blklab1:;
	//return
...
int main(int argc, char** args){
	...
	//invoke (%14) = (%1, %2, %3) 3parameter_test164:func : function(int[],int[],int[])->(int[])
	{
		...
		_14 = _func_(tmp_a, a_size, tmp_b, b_size, c, c_size, _1DARRAYSIZE_PARAM_CALLBYREFERENCE(_14));
		_14_dealloc = true;
		free(tmp_a);
		free(tmp_b);
	}
